...
/*   Updated: 2023/04/28 20:18:05 by eduarodr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	set_render(t_all_in_one *all)
{
	...
	while (++all->pos.x < X)
	{
		...
		while (++all->pos.y < Y)
		{
		all->pos.c = ft_map(all->pos.x + all->pos.xx, X - 1,
					-all->pos.zoom, all->pos.zoom);
		all->pos.d = ft_map(all->pos.y + all->pos.yy, Y - 1,
					-all->pos.zoom, all->pos.zoom);
			if (!ft_strcmp(all->vars.set, "mandelbrot"))
...
int	get_color(int iter, t_all_in_one *all)
{
	...
	return ((red << all->pos.red) | (green << all->pos.green)
		| (blue << all->pos.blue));
}
